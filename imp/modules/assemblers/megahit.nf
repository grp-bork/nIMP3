...
process metaT_megahit {
	...
	"""
	megahit -t ${task.cpus} --cpu-only -m ${task.memory} ${input_files} -o ${outdir} ${kmer_params} --bubble-level 0 --mem-flag 1
	mv -v ${outdir}/final.contigs.fa ${outdir}/${sample.id}.${stage}.transcripts.fasta
	"""
